...
const copy = {
  es: {
    ...
    introManager: 'DIAF recibe la información financiera de todas las Direcciones, administra fondos y registra la distribución y ejecución con trazabilidad.',
    introUnit: 'Carga facturas y solicitudes financieras de tu Dirección y consulta su estado dentro del flujo de DIAF.',
    loading: 'Cargando información financiera…', organization: 'Organización', funds: 'Fondos activos', pending: 'Pendientes en DIAF', paid: 'Pagadas', submittedTotal: 'Documentos recibidos',
    ...
    inbox: 'BANDEJA FINANCIERA', inboxTitle: 'Facturas y solicitudes', inboxHelpManager: 'Todo lo enviado por las Direcciones aparece aquí para revisión de DIAF.', inboxHelpUnit: 'Aquí puedes seguir las facturas y solicitudes enviadas por tu Dirección.',
    ...
    unit: 'Dirección / unidad', period: 'Período de gestión', documentType: 'Tipo de documento', vendor: 'Proveedor / beneficiario', documentNumber: 'N.º de factura o documento', documentDate: 'Fecha del documento', dueDate: 'Fecha de vencimiento', description: 'Concepto / descripción', amount: 'Monto', currency: 'Moneda', files: 'Factura y soportes', fileHelp: 'PDF, imagen, Excel o CSV · máximo 20 MB por archivo.', submit: 'Enviar a DIAF', saving: 'Guardando…',
    ...
    review: 'Gestionar', diafAction: 'GESTIÓN DIAF', reviewStatus: 'Decisión / estado', diafNotes: 'Observación de DIAF', paymentFund: 'Fondo desde el que se pagará', paymentReference: 'Referencia de pago', paymentProof: 'Comprobante de pago (opcional)', apply: 'Aplicar', attachments: 'archivos', assigned: 'Asignado a', available: 'Saldo', inflows: 'Entradas', outflows: 'Salidas',
    successInvoice: 'Documento enviado a DIAF.', successFund: 'Fondo creado.', successMovement: 'Movimiento registrado.', successTransfer: 'Transferencia registrada.', successReview: 'Estado financiero actualizado.',
  },
  en: {
    ...
    review: 'Manage', diafAction: 'FINANCE REVIEW', reviewStatus: 'Decision / status', diafNotes: 'Finance notes', paymentFund: 'Fund used for payment', paymentReference: 'Payment reference', paymentProof: 'Payment proof (optional)', apply: 'Apply', attachments: 'files', assigned: 'Assigned to', available: 'Balance', inflows: 'Inflows', outflows: 'Outflows',
  ...
  return <ManagementStandaloneShell access={access}>
    <div className="management-panel finance-page">
      ...
      {loading ? <div className="management-loading"><span /><p>{t.loading}</p></div> : <>
        ...
        {canManage && activeForm === 'fund' && <form className="management-form-card finance-form" onSubmit={saveFund}><div className="management-form-title"><div><small>DIAF</small><h2>{t.newFund.replace('＋ ','')}</h2></div><button type="button" onClick={() => setActiveForm('')}>{t.close}</button></div><div className="management-form-grid">
          ...
          <label><span>{t.assignedUnit}</span><select value={fundForm.owner_unit_id} onChange={(e) => setFundForm((c) => ({ ...c, owner_unit_id: e.target.value }))}><option value="">DIAF / institucional</option>{units.map((unit) => <option key={unit.id} value={unit.id}>{unit.code} · {unit.name}</option>)}</select></label><label><span>{t.openingBalance}</span><input type="number" min="0" step="0.01" value={fundForm.opening_balance} onChange={(e) => setFundForm((c) => ({ ...c, opening_balance: e.target.value }))} /></label>
        ...
        {canManage && activeForm === 'movement' && <form className="management-form-card finance-form compact" onSubmit={saveMovement}><div className="management-form-title"><div><small>DIAF</small><h2>{t.movement.replace('＋ ','')}</h2></div><button type="button" onClick={() => setActiveForm('')}>{t.close}</button></div><div className="management-form-grid">
        ...
        {canManage && activeForm === 'transfer' && <form className="management-form-card finance-form compact" onSubmit={saveTransfer}><div className="management-form-title"><div><small>DIAF</small><h2>{t.transfer.replace('⇄ ','')}</h2></div><button type="button" onClick={() => setActiveForm('')}>{t.close}</button></div><div className="management-form-grid">
        ...
        {canManage && activeForm === 'review' && <form className="management-form-card finance-form compact" onSubmit={saveReview}><div className="management-form-title"><div><small>{t.diafAction}</small><h2>{submissions.find((item) => item.id === reviewForm.id)?.description}</h2></div><button type="button" onClick={() => setActiveForm('')}>{t.close}</button></div><div className="management-form-grid">
          ...
          <label className="wide"><span>{t.diafNotes}</span><textarea value={reviewForm.diaf_notes} onChange={(e) => setReviewForm((c) => ({ ...c, diaf_notes: e.target.value }))} /></label>
