...
const emptyForm = () => ({ id: '', code: '', name: '', unit_type: 'directorate', parent_unit_id: '', description: '', sort_order: 0, active: true, leader: { display_name: '', email: '', operator_access_id: '' }, members: [] })
...
export default function ManagementStructurePage() {
  ...
  const editUnit = (unit) => {
    ...
    setForm({
      ...
      leader: { operator_access_id: primaryOperator?.id || '', display_name: primaryOperator?.display_name || unit.manager_name || '', email: primaryOperator?.email || unit.manager_email || '' },
      members: team,
    })
  ...
  const selectLeader = (operatorId) => {
    ...
    setForm((current) => ({ ...current, leader: operator ? { operator_access_id: operator.id, display_name: operator.display_name || '', email: operator.email || '' } : { display_name: '', email: '', operator_access_id: '' } }))
  }
  ...
  const save = async (event) => {
    ...
    if (!form.leader.display_name.trim() || !validEmail(leaderEmail)) { setError('Indica el nombre y un correo válido para la persona que lidera la unidad.'); return }
    ...
    const emails = [leaderEmail, ...cleanedMembers.map((item) => item.email)]
    ...
    const { data, error: requestError } = await supabase.rpc('admin_save_organization_unit_v2', { payload: {
      ...
      leader: { display_name: form.leader.display_name.trim(), email: leaderEmail },
  ...
  return <ManagementStandaloneShell access={access}>
    <div className="management-panel management-structure-page">
      ...
      {open && <form className="management-form-card structure-editor" onSubmit={save}>
        <div className="management-form-title"><div><small>{form.id ? 'EDITAR UNIDAD' : 'NUEVA UNIDAD'}</small><h2>{form.id ? form.name : 'Agregar al organigrama'}</h2><p>Al guardar, Edifica vinculará estas personas con la organización. Quienes todavía no tengan acceso recibirán una invitación por correo.</p></div><button type="button" onClick={reset}>Cerrar</button></div>
        ...
        <section className="structure-people-section leader-section">
          <div className="structure-section-heading"><div><small>RESPONSABLE PRINCIPAL</small><h3>¿Quién lidera esta unidad?</h3><p>Esta persona quedará como director o responsable de la unidad y como usuario de esta organización en Edifica.</p></div></div>
          <label className="existing-user-select"><span>Seleccionar un usuario ya existente (opcional)</span><select value={form.leader.operator_access_id} onChange={(event) => selectLeader(event.target.value)}><option value="">Crear o identificar por nombre y correo</option>{activeOperators.map((operator) => <option key={operator.id} value={operator.id}>{operator.display_name} · {operator.email}</option>)}</select></label>
          <div className="structure-person-grid"><label><span>Nombre y apellido *</span><input value={form.leader.display_name} onChange={(event) => setForm((current) => ({ ...current, leader: { ...current.leader, display_name: event.target.value, operator_access_id: '' } }))} required /></label><label><span>Correo electrónico *</span><input type="email" value={form.leader.email} onChange={(event) => setForm((current) => ({ ...current, leader: { ...current.leader, email: event.target.value, operator_access_id: '' } }))} required /></label><div className="structure-fixed-role"><span>Rol en esta unidad</span><strong>Director / responsable</strong></div></div>
        </section>
