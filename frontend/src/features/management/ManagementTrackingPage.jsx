...
const copy = {
  es: {
    ...
    newIndicator: '＋ Crear indicador', editIndicator: 'Editar indicador', progress: 'Registrar resultado', archive: 'Archivar', delete: 'Eliminar',
    ...
    measure: '¿Qué quieres medir? *', measureExample: 'Ej.: Personas capacitadas, iglesias participantes, ingresos anuales, presupuesto ejecutado.', metricType: '¿Qué clase de resultado vas a registrar?', aggregation: 'Cuando cargues varios resultados, ¿cómo debe quedar el total?', aggregationHelp: 'Usa “Sumar” cuando cada carga representa una parte adicional del total. Usa “Último resultado” cuando cada carga ya representa el acumulado completo hasta esa fecha.', unitLabel: '¿En qué unidad lo vas a contar?', unitExample: 'Ej.: personas, iglesias, kits, publicaciones, litros.', target: 'Meta del indicador', targetExample: 'Se registra una sola vez. Ej.: 300 personas, 85 %, 150.000 USD.', currency: 'Moneda', frequency: '¿Cada cuánto esperas actualizar este indicador?', objective: 'Objetivo institucional relacionado (opcional)', noObjective: 'Sin objetivo relacionado', objectiveHelp: 'Puedes vincularlo después.', project: 'Proyecto relacionado (opcional)', noProject: 'Sin proyecto relacionado', projectHelp: 'Puedes vincularlo después sin perder el historial.', description: '¿Qué significa este indicador?', descriptionExample: 'Aclara qué se cuenta y qué queda fuera.', source: '¿De dónde saldrá este dato?', sourceExample: 'Ej.: reporte financiero, lista de asistencia, formulario, acta o base de datos.', cancel: 'Cancelar', saveIndicator: 'Guardar indicador', saveChanges: 'Guardar cambios', saving: 'Guardando…',
    resultEyebrow: 'RESULTADO DEL PERÍODO', resultIntro: 'La meta ya está guardada. Aquí solo cargas el resultado real alcanzado; Edifica hará la comparación automáticamente.', targetReference: 'META REGISTRADA', startDate: 'Inicio del período reportado (opcional)', startDateHelp: 'Úsalo cuando el resultado corresponda a un intervalo.', endDate: 'Fecha o cierre del resultado', endDateHelp: 'Fecha hasta la cual estás reportando este resultado.', resultValue: 'Resultado logrado', amountValue: 'Monto logrado', percentageValue: 'Porcentaje logrado (%)', ratioValue: 'Valor logrado', textValue: '¿Qué resultado o información deseas reportar?', resultHelp: 'Escribe únicamente el dato real alcanzado.', yesNoValue: '¿Se cumplió?', saveAsDraft: 'Guardar como borrador', draftHelp: 'Actívalo solo si todavía no debe entrar en los cálculos.', notes: '¿Qué ocurrió durante este período?', notesExample: 'Agrega contexto, incidencias o información útil para el informe.', saveResult: 'Guardar resultado',
    board: 'TABLERO DE SEGUIMIENTO', indicators: 'indicadores', noIndicators: 'Todavía no hay indicadores para esta área y período.', targetLabel: 'Meta', achieved: 'Logrado', pending: 'Pendiente', execution: 'Ejecución', results: 'resultados cargados', indicatorSaved: 'Indicador guardado.', indicatorUpdated: 'Indicador actualizado.', resultSaved: 'Resultado registrado.', requiredResult: 'Debes registrar el resultado alcanzado antes de guardar.', indicatorArchived: 'Indicador archivado. Su historial se conserva.', indicatorDeleted: 'Indicador eliminado.', archiveConfirm: 'Este indicador dejará de aparecer en el seguimiento activo, pero conservará todos sus resultados e historial. ¿Deseas archivarlo?', deleteConfirm: 'Este indicador no tiene resultados. ¿Deseas eliminarlo definitivamente?', hasHistoryDelete: 'Este indicador ya tiene resultados. Para proteger la trazabilidad debes archivarlo en lugar de eliminarlo.', loading: 'Cargando indicadores…',
  },
  en: {
    ...
    newIndicator: '＋ Create indicator', editIndicator: 'Edit indicator', progress: 'Record result', archive: 'Archive', delete: 'Delete',
    ...
    measure: 'What do you want to measure? *', measureExample: 'Example: People trained, participating churches, annual income, budget executed.', metricType: 'What kind of result will you record?', aggregation: 'When several results are entered, how should the total be obtained?', aggregationHelp: 'Use “Add” when each entry is an additional portion of the total. Use “Latest result” when each entry already represents the full accumulated value to date.', unitLabel: 'What unit will you use?', unitExample: 'Example: people, churches, kits, posts, liters.', target: 'Indicator target', targetExample: 'Set it once. Example: 300 people, 85%, USD 150,000.', currency: 'Currency', frequency: 'How often do you expect to update this indicator?', objective: 'Related institutional objective (optional)', noObjective: 'No related objective', objectiveHelp: 'You can link it later.', project: 'Related project (optional)', noProject: 'No related project', projectHelp: 'You can link it later without losing history.', description: 'What does this indicator mean?', descriptionExample: 'Clarify what is included and excluded.', source: 'Where will this data come from?', sourceExample: 'Example: financial report, attendance list, form, minutes, or database.', cancel: 'Cancel', saveIndicator: 'Save indicator', saveChanges: 'Save changes', saving: 'Saving…',
    resultEyebrow: 'PERIOD RESULT', resultIntro: 'The target is already stored. Enter only the actual result achieved; Edifica will compare it automatically.', targetReference: 'SAVED TARGET', startDate: 'Start of reported period (optional)', startDateHelp: 'Use it when the result covers an interval.', endDate: 'Result date or period end', endDateHelp: 'Date through which this result is reported.', resultValue: 'Result achieved', amountValue: 'Amount achieved', percentageValue: 'Percentage achieved (%)', ratioValue: 'Value achieved', textValue: 'What result or information do you want to report?', resultHelp: 'Enter only the actual value achieved.', yesNoValue: 'Was it achieved?', saveAsDraft: 'Save as draft', draftHelp: 'Use it only if this result should not enter calculations yet.', notes: 'What happened during this period?', notesExample: 'Add context, incidents, or information useful for the report.', saveResult: 'Save result',
    board: 'TRACKING BOARD', indicators: 'indicators', noIndicators: 'There are no indicators for this area and period yet.', targetLabel: 'Target', achieved: 'Achieved', pending: 'Remaining', execution: 'Execution', results: 'results recorded', indicatorSaved: 'Indicator saved.', indicatorUpdated: 'Indicator updated.', resultSaved: 'Result recorded.', requiredResult: 'Enter the achieved result before saving.', indicatorArchived: 'Indicator archived. Its history is preserved.', indicatorDeleted: 'Indicator deleted.', archiveConfirm: 'This indicator will leave active tracking but all history will be preserved. Archive it?', deleteConfirm: 'This indicator has no results. Delete it permanently?', hasHistoryDelete: 'This indicator already has results. Archive it instead to preserve traceability.', loading: 'Loading indicators…',
  },
}
...
const emptyProgress = { indicator_id: '', unit_id: '', reporting_period_start: '', reporting_period_end: '', numeric_value: '', text_value: '', notes: '', status: 'submitted' }

function readLanguage() {
  try { return document.documentElement.lang === 'en' || window.localStorage.getItem('edifica-language') === 'en' ? 'en' : 'es' } catch { return 'es' }
}
...
function normalizedMethod(indicator) {
  return indicator?.aggregation_method === 'calculated' ? 'latest' : (indicator?.aggregation_method || 'sum')
}
...
function aggregateIndicator(indicator, progressRows) {
  ...
  else if (['latest', 'unique_people', 'non_aggregable'].includes(method)) {
    const row = latestProgress(rows)
    value = Number(row?.numeric_value ?? row?.numerator ?? 0)
...
function resultFieldLabel(indicator, t) {
  ...
  if (indicator.metric_type === 'percentage') return t.percentageValue
...
export default function ManagementTrackingPage() {
  ...
  const startNewIndicator = () => {
    setIndicatorForm({ ...emptyIndicator }); setIndicatorOpen(true); setProgressOpen(false); setError(''); setMessage(''); window.scrollTo({ top: 0, behavior: 'smooth' })
  }
  ...
  const saveIndicator = async (event) => {
    ...
    const payload = {
      organization_id: organizationId, management_period_id: activePeriodId, unit_id: unitId, objective_id: indicatorForm.objective_id || null, project_id: indicatorForm.project_id || null,
      name: indicatorForm.name.trim(), description: indicatorForm.description.trim() || null, metric_type: indicatorForm.metric_type, unit_label: indicatorForm.unit_label.trim() || null,
      aggregation_method: indicatorForm.aggregation_method === 'calculated' ? 'latest' : indicatorForm.aggregation_method,
      target_value: indicatorForm.target_value === '' ? (indicatorForm.metric_type === 'boolean' ? 1 : null) : Number(indicatorForm.target_value), target_text: indicatorForm.target_text.trim() || null,
      currency: indicatorForm.metric_type === 'currency' ? indicatorForm.currency : null, frequency: indicatorForm.frequency, source_note: indicatorForm.source_note.trim() || null, active: true,
    }
  ...
  const startProgress = (indicator) => {
    setProgressForm({ ...emptyProgress, indicator_id: indicator.id, unit_id: indicator.unit_id, reporting_period_end: new Date().toISOString().slice(0, 10), status: 'submitted' })
  ...
  const saveProgress = async (event) => {
    ...
    setSaving(true); setError(''); setMessage('')
    const payload = { organization_id: organizationId, indicator_id: progressForm.indicator_id, unit_id: progressForm.unit_id, reporting_period_start: progressForm.reporting_period_start || null, reporting_period_end: progressForm.reporting_period_end || null, numeric_value: progressForm.numeric_value === '' ? null : Number(progressForm.numeric_value), text_value: progressForm.text_value.trim() || null, numerator: null, denominator: null, notes: progressForm.notes.trim() || null, status: progressForm.status }
    const { error: requestError } = await supabase.from('indicator_progress').insert(payload)
    if (requestError) setError(requestError.message)
    else { setMessage(t.resultSaved); setProgressForm(emptyProgress); setProgressOpen(false); await reload() }
    setSaving(false)
  }

  const archiveIndicator = async (indicator) => {
    if (!canAdmin || !window.confirm(t.archiveConfirm)) return
    setError(''); setMessage('')
    const { error: requestError } = await supabase.from('management_indicator').update({ active: false }).eq('id', indicator.id)
    if (requestError) setError(requestError.message)
    else { setMessage(t.indicatorArchived); await reload() }
  }

  const deleteIndicator = async (indicator) => {
    if (!isSuperAdmin) return
    const hasResults = progress.some((row) => row.indicator_id === indicator.id)
    if (hasResults) { setError(t.hasHistoryDelete); return }
    if (!window.confirm(t.deleteConfirm)) return
    setError(''); setMessage('')
    const { error: requestError } = await supabase.from('management_indicator').delete().eq('id', indicator.id)
    if (requestError) setError(requestError.message)
    else { setMessage(t.indicatorDeleted); await reload() }
  }
  ...
  return <div className="management-shell">
    <aside className="management-sidebar no-print">
      ...
      <div className="management-sidebar-footer">{canAdmin && <a className="management-users-link" href="/app/admin/operators">{t.users}</a>}<div><strong>{access.organizationName || organizations.find((item) => item.id === organizationId)?.name || (language === 'en' ? 'Organization' : 'Organización')}</strong><span>{access.displayName || access.email}</span></div><button onClick={access.signOut}>{t.signOut}</button></div>
    </aside>
      ...
      <div className="management-panel">
        ...
        {progressOpen && <form className="management-form-card compact management-progress-editor" onSubmit={saveProgress}>
          ...
          <div className="management-form-grid">
            ...
            {progressIndicator?.metric_type === 'text' ? <label className="wide"><span>{t.textValue}</span><textarea value={progressForm.text_value} onChange={(event) => setProgressForm((current) => ({ ...current, text_value: event.target.value }))} required /></label> : progressIndicator?.metric_type === 'boolean' ? <label><span>{t.yesNoValue}</span><select value={progressForm.numeric_value} onChange={(event) => setProgressForm((current) => ({ ...current, numeric_value: event.target.value }))} required><option value="">—</option><option value="1">{language === 'en' ? 'Yes' : 'Sí'}</option><option value="0">No</option></select></label> : <label className="wide"><span>{resultFieldLabel(progressIndicator, t)}</span><input type="number" step="0.01" value={progressForm.numeric_value} onChange={(event) => setProgressForm((current) => ({ ...current, numeric_value: event.target.value }))} required /><small className="management-field-help">{t.resultHelp}</small></label>}
        ...
        <section className="management-indicators-card">
          ...
          {!currentIndicators.length ? <p className="management-empty">{t.noIndicators}</p> : <div className="indicator-grid">{currentIndicators.map((indicator) => {
            ...
            const target = Number(indicator.target_value || 0)
            const numericResult = result.value === null || result.value === undefined ? null : Number(result.value)
            const pending = target > 0 && numericResult !== null ? Math.max(target - numericResult, 0) : null
            const hasHistory = indicatorProgress.length > 0
            return <article key={indicator.id}>
              <header><span>{metricTypes[language][indicator.metric_type] || indicator.metric_type}</span><b>{aggregationOptions[language][normalizedMethod(indicator)] || normalizedMethod(indicator)}</b></header>
              <h3>{indicator.name}</h3>
              <div className="indicator-values" style={{ gridTemplateColumns: 'repeat(auto-fit,minmax(120px,1fr))' }}>
                <div><span>{t.targetLabel}</span><strong>{indicator.target_value === null ? (indicator.target_text || '—') : metricDisplay(indicator.target_value, indicator, language)}</strong></div>
                <div><span>{t.achieved}</span><strong>{result.text || metricDisplay(result.value, indicator, language)}</strong></div>
                {target > 0 && indicator.metric_type !== 'boolean' && indicator.metric_type !== 'text' && <div><span>{t.pending}</span><strong>{pending === null ? '—' : metricDisplay(pending, indicator, language)}</strong></div>}
              </div>
              {target > 0 && indicator.metric_type !== 'text' && <><div className="indicator-progress"><span style={{ width: `${Math.min(result.completion, 100)}%` }} /><b>{result.completion}%</b></div><small style={{ color: 'var(--mg-purple)', fontWeight: 800 }}>{t.execution}: {result.completion}%</small></>}
              <footer className="indicator-card-footer">
                <small>{frequencyOptions[language][indicator.frequency]} · {indicatorProgress.length} {t.results}</small>
                {canManageSelected && <div className="indicator-card-actions">
                  <button className="edit" type="button" onClick={() => editIndicator(indicator)}>{t.editIndicator}</button>
                  <button type="button" onClick={() => startProgress(indicator)}>{t.progress}</button>
                  {canAdmin && <button className="edit" type="button" onClick={() => archiveIndicator(indicator)}>{t.archive}</button>}
                  {isSuperAdmin && !hasHistory && <button type="button" style={{ borderColor: '#b42318', color: '#b42318', background: '#fff' }} onClick={() => deleteIndicator(indicator)}>{t.delete}</button>}
                </div>}
              </footer>
            </article>
          })}</div>}
