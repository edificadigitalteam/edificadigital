...
const DRAFT_KEY = 'edifica-monetary-draft-v2'

const content = {
  es: {
    metaTitle: 'Nueva donación monetaria | Edifica',
    ...
    sections: {
      donor: ['01', 'Donante y proyecto', 'Identifica al aliado financiador y el proyecto al que corresponde el ingreso.'],
    ...
    successTitle: 'Donación monetaria registrada',
    successCopy: 'El ingreso quedó guardado y asociado al contexto operativo seleccionado.',
    another: 'Registrar otra donación',
    panel: 'Volver al panel',
    errors: {
      organization: 'Selecciona la organización responsable.',
      evidenceRequired: 'Adjunta al menos un comprobante.',
      evidenceInvalid: 'Revisa el formato o tamaño del comprobante.',
      default: 'No fue posible guardar la donación.',
    },
  },
  en: {
    metaTitle: 'New monetary donation | Edifica',
    back: 'Back to dashboard',
    draftSaved: 'Draft saved',
    signOut: 'Use another account',
    eyebrow: 'FUNDS INTAKE',
    ...
    sections: {
      donor: ['01', 'Donor and project', 'Identify the funding partner and the project related to the income.'],
    ...
    successTitle: 'Monetary donation registered',
    successCopy: 'The income was saved and linked to the selected operational context.',
    another: 'Register another donation',
    panel: 'Back to dashboard',
    errors: {
      organization: 'Select the responsible organization.',
      evidenceRequired: 'Attach at least one payment document.',
      evidenceInvalid: 'Review the evidence format or size.',
      default: 'The donation could not be saved.',
    },
  },
}
...
const labels = {
  es: {
    organization: 'Organización *', project: 'Proyecto financiado', donorName: 'Donante / aliado *', donorType: 'Tipo de donante',
    organizationType: 'Organización', personType: 'Persona', donorContact: 'Correo o teléfono', anonymous: 'Registro anónimo',
  ...
  en: {
    organization: 'Organization *', project: 'Funded project', donorName: 'Donor / partner *', donorType: 'Donor type',
    organizationType: 'Organization', personType: 'Person', donorContact: 'Email or phone', anonymous: 'Anonymous record',
...
function loadDraft() {
  ...
  } catch { return createInitialMonetaryDraft() }
}
...
function Section({ data, children }) {
  return <section className="monetary-section unified-monetary-section"><header><span>{data[0]}</span><div><h2>{data[1]}</h2><p>{data[2]}</p></div></header>{children}</section>
}

function Field({ label, error, children, className = '' }) {
  return <label className={`intake-field ${className}`}><span className="field-label">{label}</span>{children}{error && <p className="field-error">{error}</p>}</label>
}
...
export default function UnifiedMonetaryDonationFlow() {
  ...
  const update = (name, value) => {
    setDraft((current) => {
      ...
      if (name === 'originAmount' && next.originCurrency === 'USD') {
        next.exchangeRateToUsd = '1'
  ...
  const submit = async (event) => {
    ...
    setMessage('')
    try {
      const saved = await submitMonetaryDonation({ client: supabase, draft, evidence })
      window.localStorage.removeItem(DRAFT_KEY)
      setReference(saved.reference_code)
    } catch (requestError) {
      setMessage(requestError?.message || copy.errors.default)
    } finally { setSaving(false) }
  }

  const reset = () => {
    const next = createInitialMonetaryDraft()
      ...
      <main className="intake-main monetary-main">
        ...
        <form className="monetary-form" onSubmit={submit}>
          <Section data={copy.sections.donor}><div className="monetary-grid">
            ...
            <Field label={field.donorName} error={errors.donorName} className="wide"><input value={draft.donorName} onChange={(event) => update('donorName', event.target.value)} /></Field>
            <Field label={field.donorType}><select value={draft.donorType} onChange={(event) => update('donorType', event.target.value)}><option value="organization">{field.organizationType}</option><option value="person">{field.personType}</option></select></Field>
            <Field label={field.donorContact}><input value={draft.donorContact} onChange={(event) => update('donorContact', event.target.value)} /></Field>
            <label className="monetary-check wide"><input type="checkbox" checked={draft.isAnonymous} onChange={(event) => update('isAnonymous', event.target.checked)} /><span>✓ {field.anonymous}</span></label>
          </div></Section>
