...
export default function ProjectCompliancePanel({ access }) {
  ...
  const tableOfContents = useMemo(() => buildReportTableOfContents({ hasEvidence: evidences.length > 0 }), [evidences.length])
    ...
    <div className="operations-page compliance-page">
      ...
      {!selectedProject ? <section className="operations-card"><p className="edifica-empty">Crea o selecciona un proyecto para registrar su ejecución.</p></section> : (
        <>
          ...
          <section id="report-cover" className="print-cover">
            <div className="print-cover-heading"><p className="edifica-kicker">INFORME DE CUMPLIMIENTO</p><h1>{selectedProject.name}</h1><span>{selectedProject.code} · {selectedProject.funding_partner}</span></div>
            <div className="print-cover-body">
              <div className="print-cover-gauge" style={{ '--gauge-value': `${Math.min(Math.max(averageCompliance, 0), 100)}%` }}><strong>{averageCompliance}%</strong><span>cumplimiento físico</span></div>
              <div className="print-cover-metrics">
                <article><span>Aprobado u otorgado</span><strong>{formatMoney(selectedProject.approved_budget, selectedProject.currency)}</strong></article>
                <article><span>Recibido</span><strong>{formatMoney(receivedProjectCurrency, selectedProject.currency)}</strong></article>
                <article><span>Ejecutado</span><strong>{formatMoney(investment, selectedProject.currency)}</strong></article>
                <article><span>Personas beneficiadas</span><strong>{formatNumber(beneficiaries)}</strong></article>
              </div>
            </div>
            <p className="print-cover-objective">{selectedProject.objective}</p>
            <nav className="print-cover-index" aria-label="Índice del informe">
              <span>Índice</span>
              <ol>{tableOfContents.map((entry) => <li key={entry.id}><a href={`#${entry.id}`}>{entry.label}</a></li>)}</ol>
            </nav>
          </section>
          ...
          <section className="operations-card final-report-card">
            ...
            <div id="section-financial" className="final-report-section funding-reconciliation-section">
              <div className="edifica-section-heading"><div><p className="edifica-kicker">COTEJO FINANCIERO</p><h2>Otorgado, recibido y ejecutado</h2></div><span>{linkedDonations.length} donaciones asociadas</span><a className="print-back-to-index" href="#report-cover">↑ Volver al índice</a></div>
