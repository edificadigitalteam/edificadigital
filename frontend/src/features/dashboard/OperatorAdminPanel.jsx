      ...
      {formOpen && (
        <section className="edifica-admin-form-card">
          ...
          <form className="edifica-admin-form" onSubmit={saveOperator}>
            ...
            <label><span>Organización</span><select value={form.organization_id} onChange={(event) => { const organizationId = event.target.value; setForm((current) => ({ ...current, organization_id: organizationId, role: organizationId ? (current.role === 'super_admin' ? 'operator' : current.role) : 'super_admin' })) }} disabled={!isSuperAdmin}><option value="">Sin asignar</option>{organizations.map((organization) => <option key={organization.id} value={organization.id}>{organization.name}</option>)}</select></label>
            <label><span>Rol</span><select value={form.role} onChange={(event) => setForm((current) => ({ ...current, role: event.target.value }))} disabled={!isSuperAdmin}>{form.organization_id ? <><option value="operator">Operador</option><option value="admin">Administrador</option></> : <option value="super_admin">Superadministrador</option>}</select></label>
