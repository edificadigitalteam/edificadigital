-- Allow an organizational unit leader to remain a visible contact without
-- provisioning or linking an Edifica operator account.
...
begin
  ...
  leader_email := nullif(lower(trim(coalesce(leader ->> 'email',''))), '');
  begin
    leader_create_access := coalesce((leader ->> 'create_access')::boolean, true);
  exception when others then
    raise exception using errcode = '22023', message = 'Invalid leader access choice.';
  end;

  if leader_name = '' then
    raise exception using errcode = '22023', message = 'Leader name is required.';
  end if;
  if leader_email is not null and leader_email !~ '^[^[:space:]@]+@[^[:space:]@]+\.[^[:space:]@]+$' then
    raise exception using errcode = '22023', message = 'Leader email must be valid when provided.';
  end if;
  if leader_create_access and leader_email is null then
    raise exception using errcode = '22023', message = 'Leader email is required to create access.';
  end if;

  if leader_create_access then
