...
-- Idempotent and safe for fresh environments as well as existing tenants.
...
-- Backfill objectives without referencing the UPDATE target inside a JOIN clause.
update public.institutional_objective o
set work_plan_id=p.id
from public.objective_unit_assignment a, public.unit_work_plan p
where a.objective_id=o.id
  and a.assignment_type='responsible'
  and p.unit_id=a.unit_id
  and p.management_period_id=o.management_period_id
  and o.work_plan_id is null;
