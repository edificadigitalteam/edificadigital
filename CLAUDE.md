...
- Every actionable button gets a native `title` attribute with a short description of what it does — mandatory for icon-only buttons (the accessible name fallback), expected on text buttons too for consistency. See "Button Tooltip Standard" in `docs/DESIGN.md`.
- Every module the tenant can reach carries a tenant-configurable name: a default label plus an entry in `frontend/src/features/settings/moduleLabels.js`, edited from the "Nombres de los módulos" maintainer. Renaming is configuration, so it lives in Mantenedores — never as a card inside the module it renames. See "Module Naming Standard" in `docs/DESIGN.md`.
- Every module's create/edit mechanism (inline, full page, or modal) is chosen per the decision order in "Create/Edit Record Mechanism Standard" in `docs/DESIGN.md` — not by preference, and not the same mechanism for every module. Full-page screens always render inside the dashboard shell with the sidebar navigation visible.
