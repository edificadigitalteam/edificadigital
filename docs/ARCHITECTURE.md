...
- The magic-link email always lands on the domain configured in the Supabase Dashboard's **`Authentication > URL Configuration`** for `edifydb`, regardless of what `emailRedirectTo` the client passes to `signInWithOtp` (`useOperatorAccess.js`'s `getAppRedirectUrl` already derives that value correctly from `window.location.origin` in production, falling back to `VITE_APP_URL` only when it is not a local URL). If a redirect target is not on the **Redirect URLs** allow list, Supabase silently substitutes the **Site URL** instead — this is a project setting, not something a migration or client code can change. `edifydb`'s allow list must include every domain the app is served from:
  - **Site URL**: `https://somosedificadigital.com`
  - **Redirect URLs**: `https://somosedificadigital.com/**`, `https://www.somosedificadigital.com/**`, `https://edificadigital.vercel.app/**`, `https://edificadigital-git-*-yangetzes-projects.vercel.app/**` (PR previews), plus `http://localhost:3000/**` and `http://127.0.0.1:3000/**` for local development (matches `supabase/config.toml`'s local `[auth]` block).
