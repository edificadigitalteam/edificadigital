...
| `organization_member` | People and organizations affiliated with the tenant | `member_type` is `person` or `organization`; `name` required; `email` and `phone` optional; `status` is `active` or `inactive`, so a member who leaves keeps their history |
